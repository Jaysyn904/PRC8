    ...
    if(nValue == DYNCONV_SETUP_STAGE)
    {
        ...
        if(!GetIsStageSetUp(nStage, oPC))
        {
            ...
            if(nStage == STAGE_WEAPON_CHOICE)
            {
                ...
                sHeader1 += "This will grant you weapon proficency, weapon focus and, eventually, weapon specialization in that weapon.";
                // Set the header
                SetHeader(sHeader1);
    ...
    else
    {
        ...
        else if(nStage == STAGE_CONFIRMATION)//confirmation
        {
            if(nChoice == TRUE)
            {
				...
				int nWeaponProf		= GetFeatOfWeaponType(nWeapon, FEAT_TYPE_WEAPON_PROFICENCY);
