...
{  
    ...
    // Handle EVENT_ONHIT - destroy non-magical weapons  
    ...
    // Handle EVENT_VIRTUAL_ONDAMAGED - reflex save for magical weapons  
    else if(nEvent == EVENT_VIRTUAL_ONDAMAGED) 
	{  
		object oTarget = OBJECT_SELF;  
		object oAttacker = GetLastDamager();  
		  
		// Check if damage was from a weapon  
		object oWeapon = GetLastWeaponUsed(oAttacker);  
		  
		if (GetIsObjectValid(oWeapon) && IPGetIsMeleeWeapon(oWeapon))  
		{  
			// Check if magical  
			if (PRCGetIsMagicalMeleeWeapon(oWeapon))  
			{  
				// Calculate only physical damage  
				int nPhysicalDamage = 0;  
				nPhysicalDamage += GetDamageDealtByType(DAMAGE_TYPE_BLUDGEONING);  
				nPhysicalDamage += GetDamageDealtByType(DAMAGE_TYPE_PIERCING);  
				nPhysicalDamage += GetDamageDealtByType(DAMAGE_TYPE_SLASHING);  
				  
				// Only apply reflex save if there was physical damage  
				if (nPhysicalDamage > 0)  
				{  
					// Reflex save DC 15  
					if (PRCMySavingThrow(SAVING_THROW_REFLEX, oTarget, 15))  
					{  
						// Heal back half physical damage (rounded down)  
						int nHeal = nPhysicalDamage / 2;  
