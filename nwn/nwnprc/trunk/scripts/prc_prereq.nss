...
void reqCombatMedic(object oPC)
{
    ...
    //:: Check PRC NewSpellBook classes (Fey, KotW, Archivist, 
	...
	//:: Check Gloura 
	if ((MyPRCGetRacialType(oPC) == RACIAL_TYPE_GLOURA) && (GetHasFeat(10013, oPC)))  //:: Bard_Cure_Light_Wounds: Spell-feats aren't initialized anywhere.
    {
        SetLocalInt(oPC, "PRC_PrereqCbtMed", 0);
        return;
    }
	...
	if(iCha > 10)
	{	
		if(GetIsInKnownSpellList(oPC, CLASS_TYPE_BARD, SPELL_CURE_LIGHT_WOUNDS))
			{
				SetLocalInt(oPC, "PRC_PrereqCbtMed", 0);
			}	
	}
