...
void DarkBolt(object oTarget,int nMissiles, int nDC , int nMetaMagic, object oPC)
{
   ...
   if (!PRCDoResistSpell(OBJECT_SELF, oTarget))
   {
    ...
    if (nMissiles>0)
        DelayCommand(6.2,DarkBolt(oTarget,nMissiles,nDC,nMetaMagic, oPC));
...
void main()
{
    ...
    //Fire cast spell at event for the specified target
        ...
        nMissiles--;
        if (nMissiles>0)
           DelayCommand(6.2,DarkBolt( oTarget,nMissiles,nDC,nMetaMagic, OBJECT_SELF));
