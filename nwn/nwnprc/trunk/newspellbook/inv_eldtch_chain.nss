...
void main()
{
    ...
    //FloatingTextStringOnCreature(GetName(oTarget)+" is first oTarget", oPC, FALSE);
    ...
    string sChainSkip = "ChainSkip" + ObjectToString(oPC) + sRandom + sTimeStamp;

    while(nNumTargets > 0 && GetIsObjectValid(oTarget) && !GetLocalInt(oTarget, sChainSkip))
    {
        nDam = d6(nDmgDice);
        ...
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            SetLocalInt(oTarget, sChainSkip, TRUE);
            DelayCommand(3.0, DeleteLocalInt(oTarget, sChainSkip));            

            iAttackRoll = PRCDoRangedTouchAttack(oTarget, TRUE, oPC, nAtkBns);
            if(iAttackRoll)
            {
                //Make SR Check
            ...
            if(iAttackRoll)
            {
                ...
                while(oTarget != OBJECT_INVALID && !GetIsReactionTypeHostile(oTarget) && !GetIsDead(oTarget) && !GetLocalInt(oTarget, sChainSkip))
                {
                    i++;
                    oTarget = GetNearestObject(OBJECT_TYPE_CREATURE, oPrevSource, i);
                    //FloatingTextStringOnCreature(GetName(oTarget)+" is oTarget", GetFirstPC(), TRUE);
                    //FloatingTextStringOnCreature(IntToString(GetLocalInt(oTarget, "ChainSkip"))+" is ChainSkip", GetFirstPC(), TRUE);
    ...
    if(GetHasFeat(FEAT_ELDRITCH_SCULPTOR, oPC))
    {
        if(!GetLocalInt(oPC, "UsingSecondBlast"))
        {
            ...
            UseInvocation(GetSpellId(), CLASS_TYPE_WARLOCK, 0, TRUE);
        }
        else
