...
void main()
{
    ...
    if (!GetLocalInt(oMeldshaper, "GorgonMaskLimit"))
    {
    	ActionDoCommand(SetLocalInt(oMeldshaper, "SpellIsSLA", TRUE));
    	ActionCastSpell(SPELL_FLESH_TO_STONE, GetMeldshaperLevel(oMeldshaper, CLASS_TYPE_TOTEMIST, MELD_GORGON_MASK), 0, GetMeldshaperDC(oMeldshaper, CLASS_TYPE_TOTEMIST, MELD_GORGON_MASK), METAMAGIC_NONE, CLASS_TYPE_INVALID, FALSE, FALSE, OBJECT_INVALID, FALSE);
    	ActionDoCommand(DeleteLocalInt(oMeldshaper, "SpellIsSLA")); 
    	SetLocalInt(oMeldshaper, "GorgonMaskLimit", TRUE);
    }	
}