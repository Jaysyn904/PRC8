/*:://////////////////////////////////////////////
//:: Spell Name Magic Circle against Law - On Enter
//:: Spell FileName PHS_S_MagicCirLA
//:://////////////////////////////////////////////
...
void main()
{
        ...
        GetAlignmentLawChaos(oTarget) != ALIGNMENT_CHAOTIC)
    {
        ...
        if(!GetLocalInt(oCaster, PHS_MOVING_BARRIER_START + IntToString(PHS_SPELL_MAGIC_CIRCLE_AGAINST_LAW)))
        {
            ...
            DelayCommand(0.1, PHS_MobileAOECheck(oCaster, PHS_SPELL_MAGIC_CIRCLE_AGAINST_LAW, vVector, oArea));
    ...
    // Delcare effects
    effect eDur = EffectVisualEffect(PHS_VFX_DUR_PROTECTION_LAW_MINOR);
    ...
    // Make the link Versus Lawful creatures only
    eLink = VersusAlignmentEffect(eLink, ALIGNMENT_LAWFUL, ALIGNMENT_ALL);

    // Remove all spells from Protection from law
    if(PHS_RemoveProtectionFromAlignment(oTarget, ALIGNMENT_LAWFUL, 2)) return;

    // Signal spell cast at event
    PHS_SignalSpellCastAt(oTarget, PHS_SPELL_MAGIC_CIRCLE_AGAINST_LAW, FALSE);

    // Do AOE "add subtract" thing
    PHS_AOE_OnEnterEffects(eLink, oTarget, PHS_SPELL_MAGIC_CIRCLE_AGAINST_LAW);
}
