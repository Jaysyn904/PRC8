/*:://////////////////////////////////////////////
//:: Spell Name Magic Circle against Good - On Enter
//:: Spell FileName PHS_S_MagicCirGA
//:://////////////////////////////////////////////
...
void main()
{
    ...
    if((GetRacialType(oTarget) == RACIAL_TYPE_OUTSIDER ||
        ...
        GetAlignmentLawChaos(oTarget) != ALIGNMENT_EVIL)
    {
        // If we are starting still, do not hedge back
        if(!GetLocalInt(oCaster, PHS_MOVING_BARRIER_START + IntToString(PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD)))
        {
            ...
            DelayCommand(0.1, PHS_MobileAOECheck(oCaster, PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD, vVector, oArea));
    ...
    // Remove all spells from Protection from evil
    if(PHS_RemoveProtectionFromAlignment(oTarget, ALIGNMENT_GOOD, 2)) return;

    // Signal spell cast at event
    PHS_SignalSpellCastAt(oTarget, PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD, FALSE);

    // Do AOE "add subtract" thing
    PHS_AOE_OnEnterEffects(eLink, oTarget, PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD);
}
