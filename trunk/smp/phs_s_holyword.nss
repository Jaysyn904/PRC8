...
//:://////////////////////////////////////////////
    ...
    Range: 13.33M.
    Area: Nongood creatures in a 13.33-M.-radius spread centered on you
    Duration: Instantaneous
    Saving Throw: None or Will negates; see text
    Spell Resistance: Yes

    Any nongood creature within the area that hears the holy word suffers the
...
{
    // Spell hook check.
    if(PHS_SpellHookCheck(PHS_SPELL_HOLY_WORD)) return;
    ...
    eBlindLink = EffectLinkEffects(eBlindLink, eCessate);
    // Deaf
    effect eDeafLink = EffectLinkEffects(eDeaf, eCessate);

    // Apply AOE visual
    effect eImpact = EffectVisualEffect(PHS_VFX_FNF_HOLY_WORD);
    ...
    while(GetIsObjectValid(oTarget))
    {
          ...
          !PHS_TotalSpellImmunity(oTarget))
        {
            ...
            PHS_SignalSpellCastAt(oTarget, PHS_SPELL_HOLY_WORD);
                ...
                if(PHS_SpellResistanceCheck(oCaster, oTarget, fDelay))
                {
                    ...
                    if(PHS_GetCanHear(oTarget))
                    {
                        ...
                        if(nHitDice <= nCasterLevelM1)
                        {
                            ...
                            // Apply it
                            PHS_ApplyDuration(oTarget, eBlindLink, fDuration);
