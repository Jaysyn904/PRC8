...
//:: Spell FileName PHS_S_MagicCirG
//:://////////////////////////////////////////////
...
//:://////////////////////////////////////////////
    ...
    All creatures within the area gain the effects of a protection from good
    spell, gaining +2 AC, +2 saves and immunity to possession by good creatures.

    In addition, no nonevil summoned creatures can enter the area either. You
...
{
    // Spell hook check.
    if(!PHS_SpellHookCheck(PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD)) return;
    ...
    // Declare effects
    effect eAOE = EffectAreaOfEffect(PHS_AOE_MOB_MAGCIR_GOOD);

    // Signal spell cast at event
    PHS_SignalSpellCastAt(oTarget, PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD, FALSE);

    // Remove previous castings
    PHS_RemoveSpecificEffectFromSpell(EFFECT_TYPE_AREA_OF_EFFECT, PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD, oTarget);

    // Set local integer so that the first ones will not be affected, which
    // is removed after 1.0 seconds.
    string sLocal = PHS_MOVING_BARRIER_START + IntToString(PHS_SPELL_MAGIC_CIRCLE_AGAINST_GOOD);
